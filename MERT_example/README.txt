...
To test the MERT module on the *_small dataset (5 sentences), run the command:
  ...
  java -Xmx1000m -Xms1000m -cp bin joshua.MERT.MERT -dir MERT_example -s src_small.txt -r ref_small.0.txt -cmd decoder_command_ex1.txt -cfg config_ex1.txt -decOut nbest_ex1.out -N 50 -names param_names.txt -init initial_lambdas.txt -maxIt 30 -opi 1 -v 1

The file config_ex1.txt instructs Joshua to use the .tm.gz and .lm.gz files in the trunk/example/ folder.  To use the larger ones in the trunk/example2/ folder instead, have MERT use config_ex2.txt:

  java -Xmx1000m -Xms1000m -cp bin joshua.MERT.MERT -dir MERT_example -s src_small.txt -r ref_small.0.txt -cmd decoder_command_ex2.txt -cfg config_ex2.txt -decOut nbest_ex2.out -N 50 -names param_names.txt -init initial_lambdas.txt -maxIt 30 -opi 1 -v 1
