...
/**
 * Implements decoder initialization, including interaction with
 * <code>JoshuaConfiguration</code> and <code>DecoderThread</code>.
 * 
 * @author Matt Post
...
public class JoshuaDecoder {
	...
	public static void writeConfigFile(double[] newWeights, String template,
			String outputFile, String newDiscriminativeModel) {
		try {
			int columnID = 0;
					...
					if (Regex.commentOrEmptyLine.matches(line)
						...
						if (!Regex.floatingNumber.matches(fds[fds.length - 1])) {
							throw new IllegalArgumentException(
									"last field is not a number; the field is: "
											+ fds[fds.length - 1]);
						}
			...
			if (newWeights != null && columnID != newWeights.length) {
				throw new IllegalArgumentException(
						"number of models does not match number of weights");
			}
	...
	private void initializeLanguageModels() throws IOException {
		...
		for (String lm_line : JoshuaConfiguration.lms) {
			...
			if (lm_type.equals("kenlm")) {
				...
				logger
						.warning("WARNING: using built-in language model; you probably didn't intend this");
				logger
						.warning("  Valid lm types are 'kenlm', 'berkeleylm', 'javalm' and 'none'");

				this.languageModels.add(new LMGrammarJAVA(lm_order, lm_file,
						left_equiv_state, right_equiv_state));
			}
		}
	}

	/**
	 * Initialize grammars from configuration. Line format is as follows: tm =
	 * format sent_specific span_limit grammar_file
	 * 
	 * @throws IOException
	 */
	...
	private void initializeStateComputers() {
		...
		if (JoshuaConfiguration.lm_order > 0) {
		...
		for (String featureLine : JoshuaConfiguration.features) {
			...
			if (feature.equals("lm") && !JoshuaConfiguration.lm_type.equals("none")) {
				...
				logger.info(String.format(
						"FEATURE: language model #%d, order %d (weight %.3f)", (index + 1),
						languageModels.get(index).getOrder(), weight));
			}
				...
				logger.info(String
						.format("FEATURE: lattice cost (weight %.3f)", weight));
			}
				...
				logger.info(String.format("FEATURE: phrase model %d (weight %.3f)",
						column, weight));
			}
				...
				logger.info(String.format(
						"FEATURE: arity phrase penalty: start %d, end %d (weight %.3f)",
				...
				logger.info(String
						.format("FEATURE: word penalty (weight %.3f)", weight));
			}
				...
				num_phrasal_features += 1;
				logger
						.info(String.format("FEATURE: OOV penalty (weight %.3f)", weight));
			} else {
