...
public class HyperGraphPickedState extends MultiPickedState<Vertex> {
  ...
  public boolean pick(Vertex v, boolean state) {
    if (!(v instanceof NodeVertex)) return false;
    NodeVertex node = (NodeVertex) v; // (NodeVertex) viewer.graph.getPredecessors(v).toArray()[0];
    if (viewer.graph.picked != null) viewer.graph.setSubTreeColor(viewer.graph.picked, 0);
    viewer.graph.picked = node;
    // viewer.graph.incrementHyperEdge(node);
    // viewer.setGraphLayout(new StaticLayout<Vertex,Edge>(viewer.graph, new
    // HyperGraphTransformer(viewer.graph)));
    ...
    if (node.getNode().hyperedges.size() == 0) return true;
