...
public class ManualConstraintsHandler {
	...
	private static final Logger logger = Logger
			.getLogger(ManualConstraintsHandler.class.getName());

	public ManualConstraintsHandler(Chart chart,
	...
	private void initialize(List<ConstraintSpan> constraintSpans) {
		 ...
		 * */
		/**
		 * (1) add manual rule (only allow flat rules) into the chart as constraints
		 * (2) add RHS or LHS constraint into constraintSpansForFiltering (3) add
		 * span signature into setOfSpansWithHardRuleConstraint; if the span
		 * contains a hard "RULE" constraint
		 */
			...
			for (ConstraintSpan cSpan : constraintSpans) {
				if (null != cSpan.rules()) {
					...
					for (ConstraintRule cRule : cSpan.rules()) {
						/**
						 * Note that LHS and RHS constraints are always hard, while Rule
						 * constraint can be soft or hard
						 **/
						switch (cRule.type()) {
						case RULE:
							...
							/**
							 * If the RULE constraint is hard, then we should filter all out
							 * all consituents (within this span), which are contructed from
							 * regular grammar
							 */
							if (cSpan.isHard()) {
								...
								this.spansWithHardRuleConstraint.add(new Span(cSpan.start(),
										cSpan.end()));
							}

							int arity = 0; // only allow flat rule (i.e. arity=0)
							Rule rule = this.grammarForConstructManualRule
									.constructManualRule(Vocabulary.id(cRule.lhs()),
											Vocabulary.addAll(cRule.foreignRhs()),
											Vocabulary.addAll(cRule.nativeRhs()), featureScores,
											arity);
					...
					if (shouldAdd) {
						if (logger.isLoggable(Level.INFO))
							logger.info("Adding LHS or RHS constraint for span "
						...
						this.constraintSpansForFiltering.put(
								getSpanSignature(cSpan.start(), cSpan.end()), cSpan);
					}
				}
			}
		}
	...
	/**
	 * if there are any LHS or RHS constraints for a span, then all the applicable
	 * grammar rules in that span will have to pass the filter.
	 */
	public List<Rule> filterRules(int i, int j, List<Rule> rulesIn) {
		if (null == this.constraintSpansForFiltering)
			return rulesIn;
		ConstraintSpan cSpan = this.constraintSpansForFiltering
				.get(getSpanSignature(i, j));
	...
	/**
	 * should we filter out the gRule based on the manually provided constraint
	 * cRule
	 */
	...
	/**
	 * if a span is *within* the coverage of a *hard* rule constraint, then this
	 * span will be only allowed to use the mannual rules
	 */
