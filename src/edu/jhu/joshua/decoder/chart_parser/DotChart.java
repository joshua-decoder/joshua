...
{ 
	...
	public void expand_cell(int i, int j){
		...
		int last_word=foreign_sent[j-1];//in seeding case: j=i+1, therefore, it will look for l_dot_bins[i][i]		
	...
	private void extend_dotitems_with_proved_items(int i, int k, int j, boolean start_dotitems){
		...
		for(DotItem dt : l_dot_bins[i][k].l_dot_items){//dotitem in dot_bins[i][k]: looking for an item in the right to the dot
			ArrayList<SuperItem> t_ArrayList = new ArrayList(p_chart.l_bins[k][j].get_sorted_super_items().values());//complete super-items
	...
	//create a dotitem and add it into the cell(i,j) of the dotchart
