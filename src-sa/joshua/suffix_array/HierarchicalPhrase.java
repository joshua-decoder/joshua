...
/**
 * HierarchicalPhrase is a class that represents a matched hierarchical phrase,
 * and provides the methods necessary for returning the Pattern that it matches
 * and for computing the intersection used in the suffix array lookups of 
...
public class HierarchicalPhrase extends AbstractPhrase {
	...
	/** 
	 * Represents a sequence of terminal and nonterminals as integer IDs. 
	 * The pattern is <em>not</em> rooted to a location in a corpus. 
	 */
	protected final Pattern pattern;
	
	/** Represents one index into the source corpus for each terminal that starts a contiguous sequence of terminals. */
	...
	/** the length of the matching phrase from the first terminal to the last terminal; not the pattern; this also means the length is invalid if the HP starts or ends with a nonterminal. */
	protected final int length;
	
	/** */
	protected final int sentenceNumber;
	
	// We are NOT storing this, because of cases where the HP starts with a nonterminal
	// In such cases, we want a single HP to represent all HPs that start with that nonterminal,
	// and storing the startIndex would spuriously require a list of HPs to store this.
	//protected final int startIndex;
	
	/**
	 * Constructs a hierarchical phrase from a pattern and indices into a corpus.
	 * <p>
	 * A pattern does not contain any indices into an actual corpus.
	 * For each sequence of terminals in the pattern, an index into the corpus is required.
	 * 
	 * @param pattern pattern of terminals and nonterminals
	 * @param corpusIndicesOfTerminalSequences the starting indices (in the corpus) of each contiguous sequence of terminals
	 * @param corpusArray integer array representation of a corpus
	 * @param length the length (in the corpus) of the matched hierarchical phrase; this is <em>not</em> the same as the length of the pattern
	 */
	public HierarchicalPhrase(Pattern pattern, int[] terminalSequenceStartIndices, int[] terminalSequenceEndIndices, CorpusArray corpusArray, int length) {
	...
	/**
	 * Constructs a trivially hierarchical phrase with no nonterminals.
	 * 
	 * @param span
	 * @param corpus
	 */
	...
	/**
	 * Constructs a hierarchical phrase appending the final symbol of a pattern to an existing hierarchical phrase.
	 * <p>
	 * This constructor is used during the query intersection algorithm in prefix tree construction.
	 * 
	 * @param pattern
	 * @param prefix
	 */
	...
	/**
	 * Constructs a hierarchical phrase by merging two existing hierarchical phrases.
	 * 
	 * @param pattern
	 * @param prefix
	 * @param suffix
	...
	protected HierarchicalPhrase(Pattern pattern, HierarchicalPhrase prefix, HierarchicalPhrase suffix) {
		...
		//TODO This constructor is almost certainly not correct.
		//     It currently merges the prefix and suffix as if they are adjacent.
		//     I think that they actually are supposed to overlap on all 
		//       but the first symbol of the prefix and last symbol of the suffix
		...
		// Use the sentence number and corpus array from the prefix.
		// This should be the same as that from the suffix, but we don't check.
		this.sentenceNumber = prefix.sentenceNumber;
		this.corpusArray = prefix.corpusArray;
		
		
		// Length is from the first terminal in the prefix to the last terminal in the suffix
	...
	/**
	 * Returns the index in the corpus of the first terminal in this phrase.
	 * 
	 * @return the index in the corpus of the first terminal in this phrase.
	 */
	...
	/**
	 * Returns the index in the corpus of the last terminal in this phrase.
	 * 
	 * @return the index in the corpus of the last terminal in this phrase.
	 */
	...
	/**
	 * Returns the index of the sentence in the corpus that the phrase is a part of.
	 * 
	 * @return the index of the sentence in the corpus that the phrase is a part of.
	 */
	...
	/**
	 * Returns the vocabulary from which the words in this phrase are drawn.
	 * 
	 * @resuts the vocabulary from which the words in this phrase are drawn.
	 */
	 ...
	 * 
	 * @return a space-delimited string of the terminals and nonterminals in the phrase.
	 */
	...
	/**
	 * Returns the integer word id of the 
	 * terminal or nonterminal at the specified position in the phrase.
	 * <p>
	 * This method will throw an exception if the phrase starts with a nonterminal.
	 * 
	 * @param position Index of a word in this phrase.
	 * @throws RuntimeException if the phrase starts with a nonterminal
	 * @return the integer word id of the 
	 *         terminal or nonterminal at the specified position in this phrase.
	 */
	...
	/**
	 * Returns the combined number of terminals and nonterminals in this phrase.
	 * 
	 * @return the combined number of terminals and nonterminals in this phrase.
	 */
	public int size() {
		return length;
	}
	...
	/**
	 * Indicates whether the first element of this phrase's pattern is a nonterminal.
	 *         
	 * @return <code>true</code> if this phrase starts with a nonterminal,
	...
	/**
	 * Indicates whether the last element of this phrase's pattern is a nonterminal.
	 *         
	 * @return <code>true</code> if this phrase ends with a nonterminal,
	 *         <code>false</code> otherwise.
	 */
