    ...
    for (int iter = 0; iter < miraIter; ++iter) {
      ...
      for (Integer s : sents) {
        ...
        if (needScale) { // otherwise featscale remains 1.0
          ...
          sumModelScore += java.lang.Math.abs(oraScore + predScore) / featScale; // restore the
                                                                                 // original model
                                                                                 // score
          ...
          // double newFeatScale = java.lang.Math.abs(scoreRatio*sumMetricDiff / sumModelDiff); //to
          // make sure modelScore*featScale/metricScore = scoreRatio
        ...
        for (int i = 0; i < vecPredFeat.length; i++) {
          ...
          if (featDiff.containsKey(diffFeatId)) // overlapping features
            ...
            // features only firing in the 2nd feature vector
        ...
        if (!runPercep) { // otherwise eta=1.0
          ...
          if (!evalMetric.getToBeMinimized()) {
            if (oraSelectMode == 1 && predSelectMode == 1) { // "hope-fear" selection
              /*
               * ora_score+ora_metric > pred_score+pred_metric pred_score-pred_metric >
               * ora_score-ora_metric => ora_metric > pred_metric
               */
              if (oraMetric + 1e-10 < predMetric) {
                System.err
                    .println("WARNING: for hope-fear selection, oracle metric score must be greater than prediction metric score!");
            ...
            if (oraSelectMode == 2 || predSelectMode == 3) {
              if (oraMetric + 1e-10 < predMetric) {
                System.err
                    .println("WARNING: for max-metric oracle selection or min-metric prediction selection, the oracle metric "
            ...
            if (oraSelectMode == 1 && predSelectMode == 1) { // "hope-fear" selection
              ...
              if (oraMetric - 1e-10 > predMetric) {
                System.err
                    .println("WARNING: for hope-fear selection, oracle metric score must be smaller than prediction metric score!");
                System.err.println("Something is wrong!");
              }
            }
            ...
            if (oraSelectMode == 2 || predSelectMode == 3) {
              if (oraMetric - 1e-10 > predMetric) {
                System.err
                    .println("WARNING: for min-metric oracle selection or max-metric prediction selection, the oracle metric "
          ...
          if (predSelectMode == 2) {
            if (predScore + 1e-10 < oraScore) {
              System.err
                  .println("WARNING: for max-model prediction selection, the prediction model score must be greater than oracle model score!");
              System.err.println("Something is wrong!");
            }
          }
          ...
          if (loss <= 0)
            ...
            // eta = C < loss/(featNorm*featScale*featScale) ? C :
            // loss/(featNorm*featScale*featScale); //feat vector not scaled before
      ...
      finalMetricScore = computeCorpusMetricScore(finalLambda); // compute final corpus-level metric
                                                                // score // the
