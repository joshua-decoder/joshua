...
public class ComputeNodeResult {
  ...
  public ComputeNodeResult(List<FeatureFunction> featureFunctions, Rule rule,
      List<HGNode> tailNodes, int i, int j, SourcePath sourcePath, List<StateComputer> stateComputers,
      int sentID) {
     ...
     * find them.
     */
    TreeMap<StateComputer, DPState> allDPStates = null;
    if (stateComputers != null) {
      for (StateComputer stateComputer : stateComputers) {
        ...
        if (allDPStates == null) allDPStates = new TreeMap<StateComputer, DPState>();
    ...
    for (FeatureFunction ff : featureFunctions) {
      if (ff instanceof StatefulFF) {
				StatefulFF feature = (StatefulFF)ff;
      ...
      } else if (ff instanceof StatelessFF) {
				StatelessFF feature = (StatelessFF)ff;
        transitionCost += feature.computeCost(rule, sourcePath, sentID);
      }
    }
  ...
  /**
   * This is called when making the final transition to the goal state.  This is called from
   * Cell.java.
   */ 
  ...
  public static FeatureVector computeTransitionFeatures(List<FeatureFunction> featureFunctions,
    ...
    for (FeatureFunction ff : featureFunctions) {
			...
			if (ff instanceof StatefulFF) {
				featureDelta.add(((StatefulFF)ff).computeFeatures(edge, i, j, sentID));
			} else if (ff instanceof StatelessFF) {
				featureDelta.add(((StatelessFF)ff).computeFeatures(edge.getRule(), edge.getSourcePath(), sentID));
			}
    }
  ...
  public void printInfo() {
    System.out.println("scores: " + transitionCost + "; " + viterbiCost + "; " + pruningCostEstimate);
  }
}
