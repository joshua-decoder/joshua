...
public abstract class TMGrammar {
	...
	*/	
	...
	protected static String non_terminal_regexp="^\\[[A-Z]+\\,[0-9]*\\]$";//e.g., [X,1]
...
//	TODO: we assume all the Chinese training text is lowercased, and all the non-terminal symbols are in [A-Z]+
	...
	public abstract static class Rule{
		public int rule_id = OOV_RULE_ID;
		//Rule format: [Phrase] ||| french ||| english ||| feature scores
		...
		public boolean is_oov_rule(){
			return (rule_id==OOV_RULE_ID)? true: false;
		}
		
		//maintaining sig significantly speed-up if this function is called for multiple times
		public String convert_to_string(){
			if(sig==null){
				...
				for(int i=0; i<feat_scores.length; i++)
					res.append(String.format(" %.4f", feat_scores[i]));
				sig =  res.toString();
			}
			return sig;
		}
...
//		maintaining sig significantly speed-up if this function is called for multiple times
		public String convert_to_string_no_feat_scores(){			
			StringBuffer res = new StringBuffer();
			res.append("["); res.append(Symbol.get_string(lhs)); res.append("] ||| ");
			res.append(Symbol.get_string(french)); res.append(" ||| ");
