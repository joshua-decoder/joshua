...
{
  public String root;
  public Vector<DerivationTree> branches;
  ...
  public DerivationTree(String str, int seenWords)
  {
...
//      println("ROOT: " + rootInfo);
      ...
      branches = new Vector<DerivationTree>();
      numBranches = 0;
      numTgtWords = 0;
      int len = str.length();
      int i = 0;
...
*/
        branches.add(new DerivationTree((str.substring(i_init,i_fin)).trim(),seenWords));
...
*/
      ...
      isPre = false;
      for (DerivationTree b : branches) { isPre = isPre || b.isLeaf; }
      isLeaf = false;
...
//    println("GO UP");
  ...
  public String toString()
  {
    if (isLeaf) {
      ...
      for (DerivationTree b : branches) { retStr += " " + b.toString(); }
  ...
  public String toSentence()
  {
    if (isLeaf) {
      ...
      String retStr = "";
      for (DerivationTree b : branches) { retStr += " " + b.toSentence(); }
      return retStr.trim();
    }
  }
  ...
  public String alignments()
  {
    if (isLeaf) {
      ...
      if (!isPre) {
        String retStr = "";
        for (DerivationTree b : branches) { retStr += " " + b.alignments(); }
        return retStr.trim();
      } else {
        if (numBranches == 1) {
