...
public class DotChart { 
	 ...
	 */
	public DotChart(Lattice<Integer> input, TMGrammar grammar, Chart chart) {
	 ...
	 * @param chart A CKY+ style chart in which completed span entries are stored.
	 */
	public DotChart(int[] inputSentence, TMGrammar grammar, Chart chart) {
	...
	/**
	 * two kinds of symbols in the foreign side: (1) non-terminal (e.g., X or NP); (2) CN-side terminal
	 ...
	 * add dotitems that start with the complete super-items in cell(i,j)
	 */
	...
	/**
	 * looking for (proved) theorems or axioms in the "Chart" that may apply and extend the dot pos 
	 */
	private void extend_dotitems_with_proved_items(int i, int k, int j, boolean start_dotitems){
		...
		for(DotItem dt : l_dot_bins[i][k].l_dot_items){//dotitem in dot_bins[i][k]: looking for an item in the right to the dot
			List<SuperItem> t_ArrayList = new ArrayList<SuperItem>(p_chart.l_bins[k][j].get_sorted_super_items().values());//complete super-items
			...
			for(SuperItem s_t : t_ArrayList){//see if it matches what the dotitem is looking for
				TrieNode child_tnode = dt.tnode.match_symbol(s_t.lhs);
				if (child_tnode!=null){
					if(start_dotitems==true && child_tnode.is_no_child_trienodes())continue;//TODO
