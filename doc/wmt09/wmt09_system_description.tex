...
\textbf{Deterministic Annealing (DA): } In this system, instead of using the regular MERT \cite{Och2003c} whose training objective is to minimize the one-best error, we use the deterministic annealing training procedure described in \newcite{Smith2006}, whose objective is to minimize the \emph{expected} error (together with the entropy regularization technique).
...
More details will be provided in \newcite{Li2009}. In this system, we have used both DA (for training) and VD (for decoding).
