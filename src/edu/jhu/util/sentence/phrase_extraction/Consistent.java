...
/**
 * Consistent is a implementation of PhraseExtractor which
 * is an identical phrase extraction algorithm to the one
 * that Philipp Koehn uses in to build the phrase tables for
 * his Pharaoh decoder.
 *
 * It starts with a bounding box, eliminates items that are not 
 * consistent (contain target alignments outside the source 
 * bounds), and then adds additional potential alignments by
 * adding adjacent target points which are unaligned.
 *
...
public class Consistent implements PhraseExtractor {
...
//===============================================================
	...
	public Collection getSourceAlignmentSpans(Grid alignmentPoints, int sourceStart, int sourceEnd, int sourceOffset, int targetOffset) {
		...
		if(isConsistent) {
			//Expansion:
			//we add additional alignments for all target points which are unaligned,
			//since se have no evidence that they they are not part of the alignment
			boolean[][] booleanArray = alignmentPoints.generateBooleanArray();
			int targetSentenceLength = alignmentPoints.getHeight();
			for(int i = targetStart; i >= 0 && (i == targetStart || Grid.rowEmpty(i, booleanArray)); i--) {
				for(int j = targetEnd; j <= targetSentenceLength && (j == targetEnd || Grid.rowEmpty(j-1, booleanArray)); j++) {
					//add the expanded alignment span
	...
	public Collection getTargetAlignmentSpans(Grid alignmentPoints, int targetStart, int targetEnd, int sourceOffset, int targetOffset) {
		...
		if(isConsistent) {
			...
			//since se have no evidence that they they are not part of the alignment
			boolean[][] booleanArray = alignmentPoints.generateBooleanArray();
			int sourceSentenceLength = alignmentPoints.getWidth();
			for(int i = sourceStart; i >= 0 && (i == sourceStart || Grid.columnEmpty(i, booleanArray)); i--) {
				for(int j = sourceEnd; j <= sourceSentenceLength && (j == sourceEnd || Grid.columnEmpty(j-1, booleanArray)); j++) {
					//add the expanded alignment span
					spans.add(new Span(sourceStart, sourceEnd, targetStart, targetEnd, sourceOffset, targetOffset));
				}
			}
...
//===============================================================
	...
	public static void main(String[] args)
	{
		...
		PhraseExtractor extractor = new Consistent();
