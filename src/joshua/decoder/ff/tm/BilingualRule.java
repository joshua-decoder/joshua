...
/**
 * Normally, the feature score in the rule should be *cost* (i.e., -LogP), so
 * that the feature weight should be positive
 * 
...
public class BilingualRule extends MonolingualRule {
	...
	/**
	 * Constructs a new rule using the provided parameters. The owner and rule id
	 * for this rule are undefined.
	 * 
	 * @param lhs
	 *          Left-hand side of the rule.
	 * @param sourceRhs
	 *          Source language right-hand side of the rule.
	 * @param targetRhs
	 *          Target language right-hand side of the rule.
	 * @param featureScores
	 *          Feature value scores for the rule.
	 * @param arity
	 *          Number of nonterminals in the source language right-hand side.
	 * @param owner
	 * @param latticeCost
	 * @param ruleID
	 */
	public BilingualRule(int lhs,
			int[] sourceRhs,
			int[] targetRhs,
			float[] featureScores,
			int arity,
	...
	public String toStringWithoutFeatScores() {
		StringBuffer sb = new StringBuffer();
		sb.append(Vocabulary.word(this.getLHS()));
		return sb.append(" ||| ").append(convertToString(this.getFrench()))
				.append(" ||| ").append(convertToString(this.getEnglish())).toString();
	}

	/**
	 * Two BilingualRules are equal of they have the same LHS, the same source RHS
	 * and the same target RHS.
	 * 
	 * @param o
