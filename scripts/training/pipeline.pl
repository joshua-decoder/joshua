...
if (! defined $GLUE_GRAMMAR_FILE) {
  ...
  "java -Xmx2g -cp $THRAX/bin/thrax.jar edu.jhu.thrax.util.CreateGlueGrammar -grammar $TUNE_GRAMMAR > $DATA_DIRS{tune}/grammar.glue",
  $TUNE_GRAMMAR,
...
# filter the test grammar
  ...
  if (! defined $GLUE_GRAMMAR_FILE) {
    ...
    "java -Xmx1g -cp $THRAX/bin/thrax.jar edu.jhu.thrax.util.CreateGlueGrammar -grammar $TEST_GRAMMAR > $DATA_DIRS{test}/grammar.glue",
    $TEST_GRAMMAR,
...
if (! defined $GLUE_GRAMMAR_FILE) {
  $cachepipe->cmd("glue-test-$NAME",
									"java -Xmx2g -cp $THRAX/bin/thrax.jar edu.jhu.thrax.util.CreateGlueGrammar -grammar $TEST_GRAMMAR > $DATA_DIRS{test}/grammar.glue",
									$TEST_GRAMMAR,
									"$DATA_DIRS{test}/grammar.glue");
  $GLUE_GRAMMAR_FILE = "$DATA_DIRS{test}/grammar.glue";
}
