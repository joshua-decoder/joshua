...
/**
 * (1) relative threshold pruning when the logP of a new edge (or an existing node) is worse than
 * the best by a threshold, prune it (2) when the number of node is greater than a threshold, prune
 * some nodes (3) maintain bestLogP and nodesHeap
 * */

public class BeamPruner<Obj extends Prunable> {

  /**
   * Ideally, if the goodness of an object changes, it should be removed from the heap (linear
   * time), and re-insert it (logN time). But, this is too expensive. So, instead, we will mark an
   * object as dead, and simply add a new object with the updated goodness.
   * */
  ...
  /**
   * MIN-HEAP, we put the worst-cost item at the top of the heap by manipulating the compare
   * function The only purpose of nodesHeap tis to help decide which nodes should be removed during
   * pruning.
   */
  ...
  private static final Logger logger = Logger.getLogger(BeamPruner.class.getName());
  ...
  /**
   * This will add the object, update the cutOff logP, and trigger pruningObjs
   */
  public List<Obj> addOneObjInHeapWithPrune(Obj obj) {
    this.nodesHeap.add(obj);
    // System.out.println("Add: " + obj.getPruneLogP()+ "; " +((HGNode)obj).i + "; " +
    // ((HGNode)obj).j + "; best= " + ((HGNode)obj).bestHyperedge.bestDerivationLogP);
  ...
  private List<Obj> pruningObjs() {
    if (logger.isLoggable(Level.FINEST))
      logger.finest(String.format("Pruning: heap size: %d; n_dead_items: %d",
          this.nodesHeap.size(), this.qtyDeadItems));
    ...
    if (this.nodesHeap.size() == this.qtyDeadItems) {
      ...
      /**
       * since all these objects are already dead, we do not consider them prunned objectives, so
       * return null
       */
    ...
    while (this.nodesHeap.size() - this.qtyDeadItems > maxNumObjs // bin limit pruning
         ...
         */
        || relativeThresholdPrune(this.nodesHeap.peek().getPruneLogP())) { // relative threshold
                                                                           // pruning
    ...
    /**
     * if the heap reaches its capacity, we will do more aggressive threshold pruning, by increase
     * the cutoffLogP
     * */
  ...
  private void updateCutoffLogP(double newLogP) {
    this.cutoffLogP = Support.findMax(this.cutoffLogP, newLogP - relativeThreshold);
  }

  /**
   * if the heap is already full, then we do more aggressive threshold pruning
   * */
  private void greedyUpdateCutoffLogP(double worstHeapLogP) {
    this.cutoffLogP = Support.findMax(this.cutoffLogP, worstHeapLogP - EPSILON);
  }
