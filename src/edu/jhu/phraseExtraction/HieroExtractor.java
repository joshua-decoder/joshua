...
import edu.jhu.phraseExtraction.HieroGrammarScorer_Hashtable.Rule;

import edu.jhu.joshua.decoder.Symbol;
import edu.jhu.lzfUtility.Alignment;
...
public class HieroExtractor {
	...
	public static void main(String[] args) {		
		...
		NULL_ALIGN_WRD_SYM_ID = Symbol.add_terminal_symbol(NULL_ALIGN_WRD_SYM);
		NON_TERMINAL_TAG_SYM_ID = Symbol.add_non_terminal_symbol(NON_TERMINAL_TAG_SYM);
		
		//read weights files
		eweights_table = read_weight_file("C:\\data_disk\\java_work_space\\SyntaxMT\\edu.jhu.phraseExtraction\\lex.f2e.gz");
		fweights_table = read_weight_file("C:\\data_disk\\java_work_space\\SyntaxMT\\edu.jhu.phraseExtraction\\lex.e2f.gz");
		...
		BufferedReader t_reader_zh = FileUtility.getReadFileStream("C:\\data_disk\\java_work_space\\SyntaxMT\\edu.jhu.phraseExtraction\\aligned.zh","UTF8");
		//BufferedReader t_reader_tree = FileUtility.getReadFileStream(args[1].trim(),"UTF8");
	...
	private static  HashMap read_weight_file(String file){
		...
		while((line=FileUtility.read_line_lzf(t_reader))!=null){
			...
			int id1 = edu.jhu.joshua.decoder.Symbol.add_terminal_symbol(fds[0]);
			int id2 = edu.jhu.joshua.decoder.Symbol.add_terminal_symbol(fds[1]);			
	...
	private static Rule make_and_score_rule(Alignment align, int[] phrase, ArrayList item){
		...
		for(int t=0; t<item.size(); t++){
			...
			if(item.get(t) instanceof Integer){//terminal
				...
				int nt = edu.jhu.joshua.decoder.Symbol.add_non_terminal_symbol(NON_TERMINAL_TAG_SYM+","+nt_index);//get [PHRASE,nt_index]
				rule_fwords[t] = nt;
		...
		for(int t=0, k=0; t<original_en_wrds.length; t++){
			if(original_en_wrds[t]!=INVALID_WRD_ID){
				...
				if(edu.jhu.joshua.decoder.Symbol.is_nonterminal(original_en_wrds[t])==true)
					epos[k]=INVALID_POS;
				else
					epos[k]=phrase[2]+t;
				k++;
	...
	private static Rule score_rule(Alignment align, Rule r_in, int[] fpos, int[] epos){
		...
		for(int t=0; t<r_in.french.length; t++){
			if( edu.jhu.joshua.decoder.Symbol.is_nonterminal(r_in.french[t])==false ){
				if(align.num_alignments_infor_for_french[fpos[t]]<=0)
					funaligned++;
				if(fweights!=null)
		...
		for(int t=0; t<r_in.english.length; t++){
			if( edu.jhu.joshua.decoder.Symbol.is_nonterminal(r_in.english[t])==false ){
	...
	private static void print_phrase(Alignment align, int[] phrase){
		...
		for(int t=phrase[0]; t<=phrase[1]; t++)
			str += " " + edu.jhu.joshua.decoder.Symbol.get_string(align.french_wrds[t]);
		str += " en: " + phrase[2] + "-" + phrase[3];
		for(int t=phrase[2]; t<=phrase[3]; t++)
			str += " " + edu.jhu.joshua.decoder.Symbol.get_string(align.english_wrds[t]);
		str += " nt: " + edu.jhu.joshua.decoder.Symbol.get_string(phrase[4]);
