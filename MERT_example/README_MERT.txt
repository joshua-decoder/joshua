...
with a single -h argument as follows (assuming you're in the trunk folder):

  java -cp bin joshua.MERT.MERT_runner -h
...
are that if you have enough memory to be running any respectable decoder, then
memory should not be a problem for you, since MERT probably won't need any more
memory than the decoder would.

The problem is that you may not have enough memory to support *both* the
decoder and MERT at the same time.  It might seem that this should not be an
issue, since MERT basically sits idle while the decoder is producing
translations.  So why should we be concerned about MERT needing any memory
while the decoder is running?

The answer is that a Java process does not return the memory allocated to it
until it is actually finished, even if no data structures are allocated any
memory "internally" by the process itself.  In other words, even though no
memory is needed by the end of a MERT iteration, the Java process will not
return any memory already allocated to it back to the OS.  That is why, when
the decoder is launched, it would be competing for memory with MERT, since MERT
is already hogging up quite a bit of memory that it refuses to return.

Indeed, if you rerun the example above and monitor the memory consumption of
Z-MERT, you will notice that the memory allocated to it never decreases.  This
is not a problem with such a small set (100 sentences) for a such a small
number of iterations (2), but could be problematic when scaling up.

But fear not!  When you run Z-MERT, you can instruct it to launch each
iteration as its own separate Java process.  In other words, the Z-MERT driver
would launch one external process per MERT iteration, and so the end of that
iteration is indeed the end of that individual external process.  This means
that the memory required for that iteration will indeed be returned to the OS
just before the decoder is launched.

To instruct Z-MERT to function this way, you should use the -m argument:

  java -cp bin joshua.MERT.MERT_runner -m 500 MERT_example/MERT_config_ex2.txt > MERT_example/MERT.out

The -m argument tells Z-MERT to function as explained above, and the value
tells it the maximum amount of memory it is allowed during any MERT iteration.
In this example, Z-MERT will see the -m argument and recognize that each
iteration should be launched as a separate process allowed at most 500 MB of
memory.
...
way, when the decoder is launched, it would not have to contend with any other
