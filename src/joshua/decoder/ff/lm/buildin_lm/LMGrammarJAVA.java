...
/**
 * this class implement (1) read the LM file into a Trie data structure (2) get
 * LM probablity for a given n-grm (3) get equivilent state
 * 
...
public class LMGrammarJAVA extends AbstractLM {
	...
	/*
	 * A backoff node is a hashtable, it may include: (1) probabilities for next
	 * words (2) pointers to a next-layer backoff node (hashtable); the key lookup
	 * the value is: sym_id + highestID (3) backoff weight for this node (4)
	 * suffix/prefix flag to indicate that there is ngrams start from this suffix
	 */
	...
	private static final Logger logger = Logger.getLogger(LMGrammarJAVA.class
			.getName());

	public LMGrammarJAVA(int order,
			String lm_file,
	...
	private boolean have_prefix(int[] words) {
		...
		for (; i >= 0; i--) { // reverse search
			LMHash next_layer = (LMHash) pos.get(words[i]
					+ Vocabulary.size());
			if (null != next_layer) {
				pos = next_layer;
			} else {
	...
	// estimated cost
	// O(n^2)
	public int[] leftEquivalentState(int[] original_state_wrds_in, int order,
			double[] cost) {
		...
		if (original_state_wrds.length < ngramOrder - 1) {
			for (int i = 0; i < original_state_wrds.length; i++) {
				int[] currentWords = Support.sub_int_array(original_state_wrds, 0,
						i + 1);
	...
	protected double logProbabilityOfBackoffState_helper(int[] ngram_wrds,
	...
	private void read_lm_grammar_from_file(String grammar_file)
		...
		if (logger.isLoggable(Level.FINE)) {
			logger.fine("# of bow nodes: " + g_n_bow_nodes + " ; # of suffix nodes: "
					+ g_n_suffix_nodes);
	 ...
	 * search LMHash next_layer=(LMHash) pos.get(words[i]+p_symbol.getLMEndID());
