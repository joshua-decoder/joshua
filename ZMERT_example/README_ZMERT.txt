...
specify any subset of MERT's 20-some parameters.  For a full list of those
...
supplied, that value will be used as the seed.  If "time" is provided, the seed
will the value returned by a Java System.currentTimeMillis() call at the start
of the Z-MERT run.  Either way, Z-MERT will print out the seed as part of its
output.


(4) Z-MERT Memory Requirement:
------------------------------
...
just before the decoder is launched by the Z-MERT driver.
...
way, when the decoder is launched, it would not have to compete with any other
