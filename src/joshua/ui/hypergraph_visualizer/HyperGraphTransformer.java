...
public class HyperGraphTransformer implements Transformer<Vertex, Point2D> {
  ...
  public Point2D transform(Vertex v) {
    ...
    double x, y;
    /*
     * if (v instanceof NodeVertex) { NodeVertex nv = (NodeVertex) v; int span = nv.getNode().j -
     * nv.getNode().i; y = Y_BASELINE - Y_DIST * distanceToLeaf(v); x = X_DIST * nv.getNode().i; x
     * += .5 * X_DIST * span; } else if (v instanceof LeafVertex) { LeafVertex lv = (LeafVertex) v;
     * y = Y_BASELINE; x = X_DIST * lv.getTargetPosition(); } else { Vertex node = (Vertex)
     * graph.getPredecessors(v).toArray()[0]; Point2D nodePosition = transform(node); x =
     * nodePosition.getX(); y = nodePosition.getY() + .5 * Y_DIST; }
     */
  ...
  private int distanceToLeaf(Vertex v) {
    if (graph.getSuccessorCount(v) == 0) return 0;
    int maxDistance = 0;
