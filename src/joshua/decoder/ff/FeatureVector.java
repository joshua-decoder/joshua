...
public class FeatureVector {
  ...
  public FeatureVector(String featureString, String prefix) {
    ...
    /* Read through the features on this rule, adding them to the feature vector.  Unlabeled
     * features are converted to a canonical form.
     *
    ...
    if (! featureString.trim().equals("")) {
      for (String token: featureString.split("\\s+")) {
        if (token.indexOf('=') == -1) {
          ...
          features.put(token.substring(0, splitPoint), Float.parseFloat(token.substring(splitPoint + 1)));
        }
      }   
    } 
  }
  ...
  /**
   * Subtracts the weights in the other feature vector from this one.  Note that this is not set subtraction; keys found in the other FeatureVector
   * but not in this one will be initialized with a value of 0.0f before subtraction.
   */
